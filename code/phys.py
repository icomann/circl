...
class vector:
    ...
    def ang(self):
        try:
            a = math.atan(float(self.y)/self.x)
            a = aCRad(a)
            if self.x < 0:
                a += 180
        except:
...
class physobj:
    def __init__(self, mass=10, vPos =vector(0,0), vel=vector(0,0), dir = 0):
        self.a = vector(0,0)
        self.v = vel
        self.aDir = dir
	self.lDir = self.aDir
    ...
    def movementT(self,deltat):
        self.vMov = self.v*deltat-self.a*((deltat*deltat)/2)

    def movementF(self,deltat):
        self.vMov = self.v*deltat

    def move(self):
	self.vPos += self.vMov

def rAng(pos1, pos2=vector()):
    return math.atan(float(pos1.y)-pos2.y/pos1.x-pos2.x)
